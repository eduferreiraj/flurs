...
from flurs.meta_recommender import NoMeta, FloatLR
...
if __name__ == "__main__":
    ...
    k = 60
    alphas = [1.001, 1.1]
    windows = [(100,50), (200,100)]

    data_paths = [
        "datasets\\Protocol\\CD\\last-fm-pruned.csv",
        # "datasets\\Protocol\\CD\\music-listen-pruned.csv",
        # "datasets\\Protocol\\CD\\ml-25m-gte-pruned.csv",
        "datasets\\Protocol\\CD\\netflix-gte-pruned.csv"
    ]


    for path in data_paths:
        experimenter.append(Configuration(BASE_PATH + path, MFRecommender, k=k, meta=NoMeta(), seed=0, exp_name="{}_MFRecommender".format(path.split('\\')[-1].split('.')[0])))


    data_paths = [
        "datasets\\Protocol\\CD\\nf-l2m-pruned.csv",
        # "datasets\\Protocol\\CD\\ml-2k.csv",
        # "datasets\\Protocol\\CD\\yelp-pruned.csv",
    ...
    for alpha in alphas:
        for lambda_l, lambda_s in windows:
            for path in data_paths:
                experimenter.append(Configuration(BASE_PATH + path, BRISMFRecommender, k=k, meta=NoMeta(), seed=0, exp_name="{}_BRISMFRecommender".format(path.split('\\')[-1].split('.')[0])))
