...
class ADWIN(BaseDriftDetector):
    ...
    Parameters
    ----------
    delta_c : float (default=0.002)
        The delta parameter for the ADWIN algorithm detect a drift.
    delta_w : float (default=0.005)
        The delta parameter for the ADWIN algorithm detect a warning.
    ...
    def __init__(self, delta_c = 0.002, delta_w = 0.005):
        super().__init__()
    ...
    def reset(self):
        ...
        """
        super().reset()
        self.adwin_warning = BaseADWIN(self.delta_w)
        self.adwin_change = BaseADWIN(self.delta_c)
    ...
    def get_length_estimation(self):
        ...
        """
        return self.adwin_change.get_length_estimation()
    ...
    def add_element(self, input_value):
        ...
        """
        self.adwin_warning.add_element(input_value)
        self.in_warning_zone = self.adwin_warning.detected_change()

        self.adwin_change.add_element(input_value)
        self.in_concept_change = self.adwin_change.detected_change()

class BaseADWIN(BaseDriftDetector):
    """ Adaptive Windowing method for concept drift detection.
...
class Item(object):
    ...
    def __init__(self, next_item=None, previous_item=None):
        ...
        self.max_buckets = BaseADWIN.MAX_BUCKETS
    ...
    def reset(self):
        ...
        for i in range(BaseADWIN.MAX_BUCKETS + 1):
    ...
    def compress_bucket_row(self, num_deleted=1):
        for i in range(num_deleted, BaseADWIN.MAX_BUCKETS + 1):
        ...
        for i in range(1, num_deleted+1):
            self.__clear_buckets(BaseADWIN.MAX_BUCKETS - i + 1)
